#sintaxis para definir una función
"""def NOMBRE(PARAMETROS):
    SENTENCIAS
    RETURN [EXPRESION]"""
    
#NOMBRE: es el nombre de la función
#PARAMETROS: en las funciones cuando recibe argumentos se le denomina parámetros.
#SENTENCIA: es el bloque de código
#RETURN: es una sentencia de python que le indica a la funcion que devolver cuando la llamemos
#EXPRESION: es lo que devuelve la senencia return 

def saludar():
    print("Hola, los estoy saludando")

#Para llamarla escribimos el nombre de la funcion
saludar()

def test():
    variable_test = 10
    print(variable_test)
test()

def test1():
    return print("Soy un string")
test1()

def test3():
    return[1,2,3,4,5]
print(test3())
print(test3()[1])
#Nos devuelve el numero del indice que pidamos, este caso devuelve del indice 1.

lista = test3()
print(lista[-1])

def test4():    
    return print("Hola", 30, [1,2,3,4,5,6,7,8])
test4()

def suma(numero1, numero2):
    return numero1 + numero2

r = suma (7,9)
print(r)

def suma(a,b):
    return a+b

print(suma(40,50))

"""def duplicar_valor(numero):
    numero *=2
    
numero = 10
duplicar_valor(numero)
print(numero)"""

#Np duplica lo que le pedimos ^


"""def duplicar_valor(numero):
    for i,n in enumerate(numero):
        numero[i] *=2

lista2=[10,50,100]        
duplicar_valor(lista2)
print(lista2)"""

"""def duplicar_valor(numero):
    numero*=2
    
numero = 50
duplicar_valor(numero)
print(numero)"""

def cuenta (numero):
    numero -=1
    if numero>0:
        print(numero)
        cuenta (numero)
    else:
        print("termino")
cuenta(10)

def calculo_factorial(numero):
    print("valor inicial", numero)
    if numero > 1:
        numero = numero * calculo_factorial(numero -1)
    print("valor final", numero)
    return numero 
print(calculo_factorial(5))

print(int(2.5))
print(int("307"))

print(float(25))
print(float("2.5"))

print(str(3.0))
print(round(2.8)) #REDONDEA EL NUMERO

